...
df_prompt = pd.read_parquet("/Users/lifeng/PycharmProjects/MiningMisconceptionsInMathematics/data/submission.parquet")
print(len(df_prompt))
...
llamafactory_save_file = "/data/train_data_with_ref.json"
...
for i in range(len(df_prompt)):
    ...
    label = str(df_label.loc[i, 'MisconceptionId'][0]).split('.')[0]
    ...
    if label in misconception_id:
        ...
        retrival = ""
        for i, ids in enumerate(misconception_id):
            # serial number + misconceptions
            retrival += f'{i + 1}. ' + misconceptions[int(ids)] + '\n'
